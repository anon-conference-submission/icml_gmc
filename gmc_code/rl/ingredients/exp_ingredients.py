...
def exp_config():
    ...
    if scenario == "pendulum":
        ...
        if model == "gmc":
            ...
            model_config = sacred_model.mvae_pendulum()
            model_train_config = sacred_model.mvae_pendulum_train()

        elif model == "muse":
            model_config = sacred_model.muse_pendulum()
            model_train_config = sacred_model.muse_pendulum_train()

        else:
            raise ValueError(
                "[Exp Ingredient] Model not yet implemented for Pendulum environment : "
                + model
            )
