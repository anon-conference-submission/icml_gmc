...
class PendulumMVAE(MVAE):
    def __init__(self, name, latent_dim):
        super(PendulumMVAE, self).__init__(name, latent_dim)
        self.name = name
        self.latent_dim = latent_dim
        self.image_encoder = PendulumImageGaussianEncoder(latent_dim)
        self.label_encoder = PendulumSoundGaussianEncoder(latent_dim)
        self.image_decoder = PendulumImageDecoder(latent_dim)
        self.label_decoder = PendulumSoundDecoder(latent_dim)

        self.encoders = [self.image_encoder, self.label_encoder]
        self.decoders = [self.image_decoder, self.label_decoder]

    def loss_function(self, recon_data, data, mu, logvar, train_params):

        if data[0] is not None:
            img_recon_loss = train_params['lambda_x0'] * torch.sum(F.binary_cross_entropy(recon_data[0].view(recon_data[0].size(0), -1),
        ...
        if data[1] is not None:
            sound_recon_loss = train_params['lambda_x1'] * torch.sum(
                F.mse_loss(recon_data[1].view(recon_data[1].size(0), -1),
        ...
        return torch.mean(img_recon_loss + sound_recon_loss + prior_loss)
