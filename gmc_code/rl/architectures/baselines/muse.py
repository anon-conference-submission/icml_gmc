...
from gmc_code.rl.architectures.baselines.baseline_networks import *
...
class PendulumMUSE(MUSE):
    def __init__(self, name, modality_dims, latent_dim):
        super(PendulumMUSE, self).__init__(name, modality_dims=modality_dims, latent_dim=latent_dim)

        self.name = name
        self.latent_dim = latent_dim
        self.modality_dims = modality_dims

        self.image_encoder = PendulumImageGaussianEncoder(modality_dims[0])
        self.sound_encoder = PendulumSoundGaussianEncoder(modality_dims[1])

        self.image_decoder = PendulumImageDecoder(modality_dims[0])
        self.sound_decoder = PendulumSoundDecoder(modality_dims[1])

        self.image_top_encoder = PendulumMuseTopEncoder(modality_dims[0], latent_dim)
        self.sound_top_encoder = PendulumMuseTopEncoder(modality_dims[1], latent_dim)

        self.image_top_decoder = PendulumMuseTopDecoder(latent_dim=latent_dim, out_dim=modality_dims[0])
        self.sound_top_decoder = PendulumMuseTopDecoder(latent_dim=latent_dim, out_dim=modality_dims[1])

        self.modality_encoders = [self.image_encoder, self.sound_encoder]
        self.modality_decoders = [self.image_decoder, self.sound_decoder]

        self.top_encoders = [self.image_top_encoder, self.sound_top_encoder]
        self.top_decoders = [self.image_top_decoder, self.sound_top_decoder]
    ...
    def loss_function(self, recon_data, data, mod_mus, mod_logvars, mod_zs, mod_top_zs, mu, logvar, alma_mus, alma_logvars, train_params):
        ...
        # Bottom loss terms
        image_recon_loss = train_params['lambdas'][0] * torch.sum(F.binary_cross_entropy(recon_data[0].view(recon_data[0].size(0), -1),
        ...
        if current_epoch >= 200:
            ...
            top_prior_loss = train_params['beta_top']*(float(current_epoch)/200) * (-0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp(), dim=1))
