...
from gmc_code.rl.architectures.baselines.mvae import PendulumMVAE
from gmc_code.rl.architectures.baselines.muse import PendulumMUSE
...
def setup_model(scenario, model, model_config, scenario_config):
    if model == "gmc":
        ...
        if scenario == "pendulum":
            return PendulumMVAE(
                name=model_config["model"], latent_dim=model_config["latent_dim"]
            )
        ...
        if scenario == "pendulum":
            return PendulumMUSE(
                name=model_config["model"], latent_dim=model_config["latent_dim"], modality_dims=model_config["modality_dims"]
            )
...
def setup_data_module(scenario, experiment_config, scenario_config, train_config):
    ...
    elif scenario == "pendulum" or scenario == "hyperhot":
