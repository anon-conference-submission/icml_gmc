...
from gmc_code.DelaunayComponentAnalysis._DelaunayGraph import DelaunayGraph
from typing import Optional, List
import pandas as pd
import gmc_code.DelaunayComponentAnalysis._DCA_graph_utils as graph_utils
import gmc_code.DelaunayComponentAnalysis._DCA_visualization as visualization
from gmc_code.DelaunayComponentAnalysis.loggers import logger_time, get_parameters
import json
...
class DelaunayComponentAnalysis:
    def __init__(
        self,
        dirs: ExperimentDirs,
    ...
    def preprocess_data(self, data: REData):
        ...
        if data.visualize:
            ...
            logger.info("DelaunayGraphVisualizer initialized")
            return G_visualizer
    ...
    def save_parameters(
        self, params_list: List, random_seed: int = 1111, version: int = 0,
    ):
        """
        ...
        with open(
            os.path.join(self.DCA_dir, f"DCAInput_version{version}.json"), "w",
        ) as f:
    ...
    def fit(self, data: REData):
        ...
        :param data: R and E data parameters.
        :return: Delaunay graph with obtained local and global evaluation scores.
        """
        ...
        logger.info("DelaunayGraph initialized")

        n_points = data.num_R + data.num_E
        discard_component_graph = data.discard_component_graph
        del data
        gc.collect()
        ...
        ) = self.get_Delaunay_connected_components(n_points, G.graph, G_visualizer, self.clustering_params.discard_clusterer)
        logger.info("Delaunay connected components obtained")

        G.set_first_trivial_component_idx(first_non_trivial_component)
        logger.info(
        ...
        self.analyse_Delaunay_connected_components(
            component_vertex_idx_mapping, G, G_visualizer, discard_component_graph
        )
        logger.info("Delaunay connected components analysed")

        # Save results
        self.save_DCA_logs(G)
        logger.info("DCA results saved")
        return G
    ...
    def get_Delaunay_graph(
        ...
        """
        Phase 1
        Approximates and filteres Delunay graph on the given sets of representations R and E.
        :param data: R and E data parameters.
        ...
        logger.info(
            "Delaunay graph {0} created with parameters n_itera={1} and n_iterb={2}.".format(
                os.path.join(self.root, self.graph_params.unfiltered_edges_filepath),
                self.graph_params.n_itera,
                self.graph_params.n_iterb,
            )
        )
        ...
        if self.graph_params.sphere_coverage == 1.0:
            ...
            logger.info("Unfiltered Delaunay edges of shape: %s", Delaunay_edges.shape)

        else:
            logger.info("Chosen sphere coverage: %s", self.graph_params.sphere_coverage)
            ...
            logger.info("Filtered Delaunay graph shape: %s", Delaunay_edges.shape)
        logger.info("Delaunay edges extracted")
        ...
        if visualizer is not None:
            ...
            logger.info("Delaunay edges visualized")
        return G
    ...
    def get_Delaunay_connected_components(
        ...
        discard_clusterer: Optional[bool] = False,
    ):
        """
        Phase 2
        ...
        if visualizer is not None:
            ...
            logger.info("Delaunay connected components visualized")
            visualizer.xlim = xlim
            visualizer.ylim = ylim
            logger.info(f"DelaunayGraphVisualizer updated xlim={xlim} and ylim={ylim}")
        ...
        (
            ...
            self.root, input_array_labels, self.clustering_params, n_points,
        )
        logger.info("Delaunay connected components extracted")
        gc.collect()
        
        if discard_clusterer:
            path = os.path.join(self.root, self.clustering_params.clusterer_filepath)
            os.remove(path)
            logger.info(f"Clusterer removed from {path}")
    ...
    def analyse_Delaunay_connected_components(
        ...
        for comp_idx, comp_vertices in component_vertex_idx_mapping.items():
            ...
            print('analyse_Delaunay_connected_components')
            print(subgraph_RE_comp.nodes())
            ...
            if visualizer is not None and comp_idx < G.first_trivial_component_idx:
                ...
                logger.info(f"Delaunay connected component {comp_idx} visualized")
            subgraph_R_comp, subgraph_E_comp = graph_utils._extract_RE_subgraphs(
                subgraph_RE_comp, G.num_R, G.num_E
            )
            print(subgraph_R_comp.nodes())
            print(subgraph_E_comp.nodes())
            (
                comp_R_idxs,
                comp_E_idxs,
                comp_consistency,
            ...
            logger.info(f"Delaunay connected component {comp_idx} analyzed")

            if discard_component_graph:
                subgraph_RE_comp = None
            ...
            logger.info(f"DelaunayGraph updated local stats with component {comp_idx}")

        if visualizer is not None:
            visualization._plot_isolated_components(
                G, visualizer, self.visualization_dir
            )
            logger.info("Isolated Delaunay connected components")
            ...
            logger.info("distilled Delaunay edges visualized")

        G.update_global_stats()
        G.save_stats()
        logger.info(f"DelaunayGraph updated global stats")
    ...
    def save_DCA_logs(self, G: DelaunayGraph):
        ...
        logger.info(f"DelaunayGraph network_stats saved")

        path = os.path.join(self.results_dir, "components_stats.pkl")
        with open(path, "wb") as f:
            pickle.dump(G.comp_stats, f)
        logger.info(f"DelaunayGraph components_stats saved")
    ...
    def process_query_points(
        ...
        logger.info("Loaded existing DelaunayGraph")
        ...
        if assign_to_comp:
            ...
            logger.info("Query points assigned to connected components")
            return query_points_comp_labels, considered_comp_idx_list

        elif assign_to_RE:
            query_points_nclosest_init_point_idxs = self.assign_query_points_to_closest_init_point(
                init_data, query_data, return_len=return_len
            )
            logger.info("Query points assigned to closest RE point")
            ...
            logger.info("Query points assigned to closest R point")
            return query_points_nclosest_init_point_idxs
    ...
    def assign_query_points_to_components(
        ...
        self.save_DCA_logs(G)
        logger.info("Average edge length per (non-trivial) component extracted")
        ...
        if consider_several_assignments:
            ...
            extra_assignments_by_num_edges = two_assignments_sorted_by_num_edges.drop_duplicates(
    ...
    def get_query_Delaunay_edges_stats(self, init_data: REData, query_data: QueryData):
        ...
        try:
            ...
            logger.info("Unfiltered query Delaunay edges extracted")
            ...
            logger.info("query Delaunay edges extracted")
