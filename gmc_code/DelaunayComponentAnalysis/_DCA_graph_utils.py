...
import logging
from gmc_code.DelaunayComponentAnalysis.loggers import logger_time
import gc
...
def _approximate_Delaunay_edges(
    ...
    if not os.path.isfile(unfiltered_Delaunay_edges_filepath):
        os.system(
            "{0} {1} --task delaunay --niter_a {2} --niter_b {3} --out {4} --out_dist {5}".format(
                os.path.join(graph_params.executable_filepath, '../DelaunayComponentAnalysis/./VoronoiClassifier_cl'),
                os.path.join(root, input_array_filepath),
                graph_params.n_itera,
                graph_params.n_iterb,
...
def _filter_Delaunay_edges(
    ...
    if not os.path.isfile(filtered_Delaunay_edges_filepath):
        ...
        logger.info("Unfiltered edges loaded")
        logger.info("Chosen sphere coverage: %s", graph_params.sphere_coverage)
        logger.info(
        ...
        logger.info("Vertex stats dict created")
        ...
        if n_points > 20000:
            ...
            gc.collect()
            logger.info("Unfiltered edges deleted")
        ...
        for vtx_idx, vtx_data in vertex_stats.items():
            ...
            if len(vtx_data) != 0:
                ...
                vtx_data[:, 1] = np.divide(
                    vtx_data[:, 1], graph_params.n_itera * graph_params.n_iterb * 2
                ...
                while (
                    ...
                    idx += -1
        logger.info("Significant edges extracted")

        # Needed when working with large datasets
        if n_points > 20000:
            del vertex_stats
            gc.collect()
            logger.info("Vertex stats dict deleted")

        filtered_significant_edge_idxs, _ = np.unique(
            significant_edge_idxs, return_counts=True
        )
        logger.info("Unique significant edges extracted")

        # Needed when working with large datasets
        if n_points > 20000:
            del significant_edge_idxs
            gc.collect()
            logger.info("Duplicate significant edges deleted")
            unfiltered_Delaunay_edges = np.load(unfiltered_Delaunay_edges_filepath)
            unfiltered_Delaunay_edges_len = np.load(
                unfiltered_Delaunay_edges_len_filepath
            )
            logger.info("Unfiltered edges loaded again")
        ...
        logger.info("Filtered Delaunay edges shape: %s", filtered_Delaunay_edges.shape)
...
def _distil_Delaunay_connected_components(
    ...
    if os.path.isfile(clusterer_path):
        ...
        logger.info("Minimum spanning tree extracted")
        ...
        logger.info("Single linkage tree extracted")
        ...
        logger.info("HDBSCAN clusterer saved")
...
def _sort_Delaunay_connected_components(
    ...
    outlier_idxs = np.arange(
        max(input_data_labels) + 1, max(input_data_labels) + 1 + n_outliers,
    )
    first_trivial_component_idx = max(input_data_labels) + 1
    input_data_labels[input_data_labels == -1] = outlier_idxs
    logger.info("Outliers relabeled")
    logger.info("Number of outliers: {0}".format(n_outliers))
    logger.info(
    ...
    np.save(
        os.path.join(root, clustering_params.input_array_labels_filepath), relabel,
    )
    logger.info("Connected components relabeled by size")

    del input_data_labels
    gc.collect()
    logger.info("Input data labels saved and removed from memory")
    ...
    logger.info("Component vertex id mapping created")
...
def _approximate_query_Delaunay_edges(
    ...
    if not os.path.isfile(query_Delaunay_edges_filepath):
        os.system(
            "./VoronoiClassifier_cl {0} {1} --task delaunay --niter_a {2} --niter_b {3} --out {4} --out_dist {5}".format(
                input_array_filepath,
                query_input_array_filepath,
                graph_params.n_itera,
                2 * graph_params.n_iterb,
