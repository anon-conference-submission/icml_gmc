...
class ExperimentDirs:
    ...
    precomputed_folder: str = "precomputed"  # name of the folder where precomputed files are stored
...
class QueryData:
    ...
    query_input_array_comp_assignment_filename: str = "query_input_array_comp_assignment.npy"
    query_input_array_point_assignment_filepath: str = field(init=False)

    query_input_array_considered_comp_list_filename: str = "query_input_array_considered_comp_list.npy"
    query_input_array_considered_comp_list_filepath: str = field(init=False)

    query_input_array_point_assignment_filename: str = "query_input_array_point_assignment.npy"
    query_input_array_point_assignment_filepath: str = field(init=False)
...
class DelaunayGraphParams:
    ...
    T: int = 10 ** 4
    n_itera: int = field(init=False)
    n_iterb: int = field(init=False)
    ...
    def __post_init__(self):
        self.unfiltered_edges_filepath = os.path.join(
            self.unfiltered_edges_dir, self.unfiltered_edges_filename,
        )
        self.unfiltered_edges_len_filepath = os.path.join(
            self.unfiltered_edges_dir, self.unfiltered_edges_len_filename,
        )
        self.filtered_edges_filepath = os.path.join(
            self.filtered_edges_dir, self.filtered_edges_filename,
        )
        self.filtered_edges_len_filepath = os.path.join(
            self.filtered_edges_dir, self.filtered_edges_len_filename,
        )
        if self.T < 100:
            self.n_itera = self.T
            self.n_iterb = 1
...
class HDBSCANParams:
    ...
    discard_clusterer: bool = True

    def __post_init__(self):
        self.clusterer_filepath = os.path.join(
            self.clusterer_dir, self.clusterer_filename,
        )
        self.input_array_labels_filepath = os.path.join(
            self.clusterer_dir, self.input_array_labels_filename,
        )
            ...
            "handlers": {
                ...
                "DCA_stats": {
                    ...
                    "filename": os.path.join(
                        self.experiment_dir,
                        f"experiment_info_version{self.version}.log",
                    ),
                    "mode": "w",
                },
                "DCA_results": {
                    ...
                    "formatter": "time_only",
                    "filename": os.path.join(
                        self.experiment_dir, f"DCA_results_version{self.version}.log",
                    ),
                    "mode": "w",
                },
                "DCA_elapsed_time": {
                    ...
                    "filename": os.path.join(
                        self.experiment_dir,
                        f"DCA_elapsed_time_version{self.version}.log",
                    ),
                    "mode": "w",
                },
            },
