...
from gmc_code.DelaunayComponentAnalysis.schemes import DelaunayGraphVisualizer
import numpy as np
import os
...
def _plot_UMAP_components(
    G: DelaunayGraph,
    experiment_id,
    input_array_filepath: str,
    input_array_labels_filepath: str,
    visualization_root: str,
):
    modality1, modality2 = experiment_id.split("_")

    input_array = np.load(input_array_filepath)
    input_array_comp_labels = np.load(input_array_labels_filepath)
    ...
    labels = np.where(
        input_array_comp_labels <= G.first_trivial_component_idx,
        input_array_comp_labels,
        G.first_trivial_component_idx + 1,
    )

    for comp in range(G.first_trivial_component_idx + 1):
        comp_idx = np.where(labels == comp)[0]
        R_idx = comp_idx[np.where(comp_idx < G.num_R)[0]]
        E_idx = comp_idx[np.where(comp_idx >= G.num_R)[0]]
        ...
        plt.legend()
        plt.title(
            "{3}-{4}, comp {0}: R size {1}, E size {2}".format(
                comp, R_idx.shape[0], E_idx.shape[0], modality1, modality2
            )
        )
        name = f"{experiment_id}_comp{comp}"
        plt.savefig(os.path.join(visualization_root, name))
        plt.clf()
    ...
    plt.clf()
    plt.scatter(
        embedding[no_outliers_idx, 0],
        embedding[no_outliers_idx, 1],
        c=labels[no_outliers_idx],
        cmap="Spectral",
        s=5,
    )
    plt.gca().set_aspect("equal", "datalim")
    plt.colorbar(
        boundaries=np.arange(G.first_trivial_component_idx + 1) - 0.5
    ).set_ticks(np.arange(G.first_trivial_component_idx))
    plt.title(f"{modality1}-{modality2}")
    name = f"{experiment_id}_all"
    plt.savefig(os.path.join(visualization_root, name))

    R_idx = no_outliers_idx[np.where(no_outliers_idx < G.num_R)[0]]
    E_idx = no_outliers_idx[np.where(no_outliers_idx >= G.num_R)[0]]
    plt.clf()
    plt.scatter(
        embedding[R_idx, 0],
        embedding[R_idx, 1],
    ...
    name = f"ICML_{experiment_id}_all_RE"
    plt.savefig(os.path.join(visualization_root, name))
